...
Precise per-request energy attribution is challenging because power is sampled continuously while token usage is reported discretely, often only after a request completes. Multiple concurrent requests may overlap in time. Our design therefore targets aggregate attribution over two windows. The session window covers the full profiler lifetime, which is useful for long-running monitoring. The active window covers periods when at least one request is in flight, which reduces idle-time dilution in short experiments. This choice provides an actionable operational signal while avoiding overclaiming exact per-request energy accounting.
...
\texttt{in-flight requests} \(\rightarrow\) \texttt{active window}\\
\vspace{0.3em}
\end{minipage}
}
...
\begin{itemize}
    ...
    \item active-window joules per token;
    \item active-window kWh per million tokens;
...
For short experimental runs, the active-window variant uses only intervals in which at least one request is in flight. This reduces the effect of idle time before traffic starts or after traffic completes.
...
The proxy can sample all GPUs or a user-selected subset with \texttt{--gpus}. This is important for multi-GPU cluster nodes where only a subset of devices is assigned to a job. The implementation also tracks in-flight request counts to compute active-window energy metrics. The repository includes a mock OpenAI-compatible server and a traffic generator to validate the token-accounting path before running a real LLM server.
...
\begin{table}[t]
\centering
\caption{Minimal experiment matrix for the first HPEC draft.}
\label{tab:matrix}
\begin{tabular}{ll}
\toprule
Dimension & Values \\
\midrule
GPU & A100, H100, H200 \\
Server & vLLM first; SGLang optional \\
Primary point & concurrency 4, max output 256 \\
Small sweep & concurrency 1, 4, 8 \\
Metrics & tokens/s, W, active J/token, active kWh/1M \\
\bottomrule
\end{tabular}
\end{table}

Table~\ref{tab:matrix} summarizes the minimal first-pass matrix. For each GPU, the first run uses a matched model, prompt, concurrency, and output length. We then run both direct and proxied traffic on the A100 system to estimate monitoring overhead. The traffic generator records request counts, errors, average latency, p50 latency, p95 latency, and total tokens. The profiler records both session-level and active-window energy metrics.
